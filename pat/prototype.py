	...
	for temp in data_content:
		...
		for i in temp_list:
			if i.find("http")==-1 and i.find("www.")==-1 and i.find("#")==-1 and i.find("@")==-1:
				...
				a = ''.join(ch for ch, sencond in itertools.groupby(i))
				cache += a + " " 
...
def lemmatizer(data):
	...
	for item in data:
		...
		for word in word_list:
			words = words + " " + lmtzr.lemmatize(word)
...
filtered_data = pre_process(data_content)

filtered_data = lemmatizer(filtered_data)
